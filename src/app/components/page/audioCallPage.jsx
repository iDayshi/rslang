...
const AudioCallPage = () => {
  ...
  const spellWord = () => {
    const playWord = new Audio(
      `http://localhost:8080/${quizWords[correctPosition].audio}`
    );
    playWord.play();
  };
  ...
  const spellSentense = () => {
    const playWord = new Audio(
      `http://localhost:8080/${quizWords[correctPosition].audioExample}`
    );
    playWord.play();
  };
  ...
  const handleCheck = (isTrue) => {
    isTrue ? setView("correct_answer") : setView("wrong_answer");
  ...
  const makeWords = () => {
    ...
    console.log(rightWordPosition);
    console.log(correctPosition);
    quizWords1.forEach((el) => {
      el.right = false;
    });
    ...
    return (
      <div className="container audiocall_page">
        ...
        <h4 className="audiocall_header_small">
          Вопрос {wordCounter + 1} из 20. Счет: {score}
        </h4>
        ...
        <section>
          <div className="audiocall_wordcard">
            ...
            <div>
              ...
              <span>
                {" "}
                {quizWords[correctPosition].word}
                {" - "}
                {quizWords[correctPosition].wordTranslate}
              </span>
            </div>
            <button
    ...
    return (
      <div className="container">
        ...
        <section className="audiocoll_results">
          ...
          <div>
            {Math.floor((100 * correctAnswers.length) / 20)} % правильных
            ответов. {score} из 20 слов верны
          </div>
          <div>
